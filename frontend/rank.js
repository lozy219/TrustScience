...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png ?_v=daoheshenyuzhuanjin`;
...
$.get('data/nickname.json ?_v=daoheshenyuzhuanjin')
  .done(data => {
...
$.get('data/filename.json ?_v=daoheshenyuzhuanjin')
  .done(data => {
    filenames = data;
    $.get('data/data.json ?_v=daoheshenyuzhuanjin')
      .done(stats => {
        // populate stas array
        statsData = stats;

...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=guiqie`;
...
$.get('data/nickname.json?_v=guiqie')
  .done(data => {
...
$.get('data/filename.json?_v=guiqie')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=guiqie')
      .done(stats => {
        // populate stas array
        statsData = stats;

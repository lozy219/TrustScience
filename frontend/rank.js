...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=yuzaohou`;
...
$.get('data/nickname.json?_v=yuzaohou')
  .done(data => {
...
$.get('data/filename.json?_v=yuzaohou')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=yuzaohou')
      .done(stats => {
        // populate stas array
        statsData = stats;

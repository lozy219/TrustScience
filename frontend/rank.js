...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=lianyou`;
...
$.get('data/nickname.json?_v=lianyou')
  .done(data => {
...
$.get('data/filename.json?_v=lianyou')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=lianyou')
      .done(stats => {
        // populate stas array
        statsData = stats;

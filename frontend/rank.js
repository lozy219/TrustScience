...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=baimugui`;
...
$.get('data/nickname.json?_v=baimugui')
  .done(data => {
...
$.get('data/filename.json?_v=baimugui')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=baimugui')
      .done(stats => {
        // populate stas array
        statsData = stats;

...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=jiegeng`;
...
$.get('data/nickname.json?_v=jiegeng')
  .done(data => {
...
$.get('data/filename.json?_v=jiegeng')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=jiegeng')
      .done(stats => {
        // populate stas array
        statsData = stats;

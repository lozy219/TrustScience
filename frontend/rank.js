...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=baizangzhu`;
...
$.get('data/nickname.json?_v=baizangzhu')
  .done(data => {
...
$.get('data/filename.json?_v=baizangzhu')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=baizangzhu')
      .done(stats => {
        // populate stas array
        statsData = stats;

...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=yaodaoji`;
...
$.get('data/nickname.json?_v=yaodaoji')
  .done(data => {
...
$.get('data/filename.json?_v=yaodaoji')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=yaodaoji')
      .done(stats => {
        // populate stas array
        statsData = stats;

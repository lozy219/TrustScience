...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=yuzaoqian`;
...
$.get('data/nickname.json?_v=yuzaoqian')
  .done(data => {
...
$.get('data/filename.json?_v=yuzaoqian')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=yuzaoqian')
      .done(stats => {
        // populate stas array
        statsData = stats;

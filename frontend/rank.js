...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=tongge`;
...
$.get('data/nickname.json?_v=tongge')
  .done(data => {
...
$.get('data/filename.json?_v=tongge')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=tongge')
      .done(stats => {
        // populate stas array
        statsData = stats;

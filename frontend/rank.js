...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=shantu`;
...
$.get('data/nickname.json?_v=shantu')
  .done(data => {
...
$.get('data/filename.json?_v=shantu')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=shantu')
      .done(stats => {
        // populate stas array
        statsData = stats;

...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=baqidashe`;
...
$.get('data/nickname.json?_v=baqidashe')
  .done(data => {
...
$.get('data/filename.json?_v=baqidashe')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=baqidashe')
      .done(stats => {
        // populate stas array
        statsData = stats;

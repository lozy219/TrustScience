...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=yuzaozhong`;
...
$.get('data/nickname.json?_v=yuzaozhong')
  .done(data => {
...
$.get('data/filename.json?_v=yuzaozhong')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=yuzaozhong')
      .done(stats => {
        // populate stas array
        statsData = stats;

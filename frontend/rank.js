...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=sanweihu`;
...
$.get('data/nickname.json?_v=sanweihu')
  .done(data => {
...
$.get('data/filename.json?_v=sanweihu')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=sanweihu')
      .done(stats => {
        // populate stas array
        statsData = stats;

...
const renderTable = () => {
  for (let entry of statsArr) {
    ...
    const filename = `resources/pixyys/${filenames[name]}.png?_v=yujuchong`;
...
$.get('data/nickname.json?_v=yujuchong')
  .done(data => {
...
$.get('data/filename.json?_v=yujuchong')
  .done(data => {
    filenames = data;
    $.get('data/data.json?_v=yujuchong')
      .done(stats => {
        // populate stas array
        statsData = stats;
